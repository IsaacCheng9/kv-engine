...
SSTableReader::SSTableReader(const std::string &path) {
  ...
  pread_exact(fd_, &index_offset, sizeof(index_offset), footer_start);
  pread_exact(fd_, &bloom_filter_offset, sizeof(bloom_filter_offset),
              footer_start + sizeof(uint64_t));
  pread_exact(fd_, &bloom_filter_size, sizeof(bloom_filter_size),
  ...
  pread_exact(fd_, bloom_filter_data.data(), bloom_filter_size,
              bloom_filter_offset);
  ...
  while (cursor < bloom_filter_offset) {
    uint32_t key_length;
    pread_exact(fd_, &key_length, sizeof(key_length), cursor);
    cursor += sizeof(key_length);

    std::string key(key_length, '\0');
    pread_exact(fd_, key.data(), key.size(), cursor);
    cursor += key.size();

    uint64_t data_offset;
    pread_exact(fd_, &data_offset, sizeof(data_offset), cursor);
...
SSTableReader::get(std::string_view key) {
  ...
  pread_exact(fd_, &value_length, sizeof(value_length), cursor);
  ...
  pread_exact(fd_, value.data(), value_length, cursor);
...
bool SSTableReader::next_entry(std::string &out_key,
  ...
  uint32_t key_length;
  pread_exact(fd_, &key_length, sizeof(key_length), read_position_);
  read_position_ += sizeof(key_length);

  out_key.resize(key_length);
  pread_exact(fd_, out_key.data(), key_length, read_position_);
  read_position_ += key_length;

  uint32_t value_length;
  pread_exact(fd_, &value_length, sizeof(value_length), read_position_);
  ...
  if (value_length == UINT32_MAX) {
    ...
    std::string value(value_length, '\0');
    pread_exact(fd_, value.data(), value_length, read_position_);
