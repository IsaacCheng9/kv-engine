...
void SSTableWriter::add_entry(std::string_view key,
  ...
  write_all(fd_, &key_length, sizeof(key_length));
  write_all(fd_, key.data(), key.size());

  if (value.has_value()) {
    uint32_t value_length = value->size();
    write_all(fd_, &value_length, sizeof(value_length));
    ...
    write_all(fd_, &tombstone_marker, sizeof(tombstone_marker));
  }
...
void SSTableWriter::finalise() {
  ...
  for (const auto &[key, offset] : key_to_offset_) {
    ...
    write_all(fd_, &key_length, sizeof(key_length));
    write_all(fd_, key.data(), key.size());
    write_all(fd_, &offset, sizeof(offset));
  }
  ...
  write_all(fd_, bloom_bytes.data(), bloom_bytes.size());
  uint64_t bloom_size = bloom_bytes.size();

  write_all(fd_, &index_offset, sizeof(index_offset));
  write_all(fd_, &bloom_offset, sizeof(bloom_offset));
  write_all(fd_, &bloom_size, sizeof(bloom_size));
  fsync(fd_);
  finalised_ = true;
}
